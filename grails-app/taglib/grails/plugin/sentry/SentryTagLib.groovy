...
class SentryTagLib {
    ...
    def traceMeta = { attrs, body ->
        ...
        if (sentryTrace) {
            out << "<meta name=\"${SentryTraceHeader.SENTRY_TRACE_HEADER}\" content=\"${sentryTrace}\" />"
        }
    }
    ...
    def traceLink = { attrs, body ->
        ...
        if (sentryTraceId && linkPrefix) {
            out << """<a href="${linkPrefix}/performance/trace/${sentryTraceId}" target="_blank class="sentry-trace-link">"""
            out << (body() ?: """
